    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          Positioned(bottom: -100, left: -100, child: _glow(300, AppColors.accent.withValues(alpha: 0.15))),
                    ...
                    children: [
                      ...
                      CPPressable(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(color: AppColors.primary, borderRadius: BorderRadius.circular(100), boxShadow: [BoxShadow(color: AppColors.primary.withValues(alpha: 0.3), blurRadius: 12)]),
                          child: Row(
                            children: [
                              const Icon(Icons.add_alert_rounded, size: 20, color: Colors.white),
                              const SizedBox(width: 8),
                              Text('Broadcast', style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.w900, color: Colors.white, letterSpacing: -0.5)),
                            ],
                          ),
                        ),
                      ),
                      ...
                      return CPPressable(
                        ...
                        child: AnimatedContainer(
                          ...
                          decoration: BoxDecoration(
                            color: isActive ? _filterColors[i] : (isDark ? Colors.white.withValues(alpha: 0.05) : Colors.black.withValues(alpha: 0.05)),
                            borderRadius: BorderRadius.circular(100),
                            border: Border.all(color: isActive ? Colors.transparent : (isDark ? Colors.white10 : Colors.black.withValues(alpha: 0.12))),
                            boxShadow: isActive ? [BoxShadow(color: _filterColors[i].withValues(alpha: 0.3), blurRadius: 12)] : [],
                          ),
                          alignment: Alignment.center,
                          child: Text(_filters[i].toUpperCase(), style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w900, color: isActive ? Colors.white : (isDark ? Colors.white54 : Colors.black54), letterSpacing: 0.5)),
                        ),
                      );
                    },
                  ),
                              ...
                              : ListView.separated(
                                  ...
                                  itemBuilder: (context, i) {
                                    ...
                                    return CPGlassCard(
                                      ...
                                      borderRadius: 24,
                                      border: Border.all(color: pinned ? color.withValues(alpha: 0.3) : (isDark ? Colors.white.withValues(alpha: 0.05) : Colors.black.withValues(alpha: 0.05))),
                                      child: Column(
                                        ...
                                        children: [
                                          Row(children: [
                                            Container(
                                              ...
                                              child: Text(category.toUpperCase(), style: GoogleFonts.inter(fontSize: 9, fontWeight: FontWeight.w900, color: color, letterSpacing: 1.0)),
                                            ),
                                            const Spacer(),
  ...
  Future<void> _deleteAnnouncement(String id) async {
    ...
    final conf = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        backgroundColor: isDark ? AppColors.eliteDarkBg : AppColors.eliteLightBg,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(24), side: BorderSide(color: isDark ? Colors.white12 : Colors.black.withValues(alpha: 0.12))),
        title: Row(
          children: [
            Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: AppColors.error.withValues(alpha: 0.1), shape: BoxShape.circle), child: const Icon(Icons.warning_rounded, color: AppColors.error)),
            const SizedBox(width: 12),
            Text('Retract Broadcast?', style: GoogleFonts.inter(fontSize: 18, fontWeight: FontWeight.w800, color: isDark ? Colors.white : AppColors.deepNavy, letterSpacing: -0.5)),
          ],
        ),
        content: Text('This action will permanently remove this broadcast from all feeds. Proceed?', style: GoogleFonts.inter(fontSize: 14, color: isDark ? Colors.white54 : Colors.black54)),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: Text('Cancel', style: GoogleFonts.inter(fontWeight: FontWeight.w600, color: isDark ? Colors.white54 : Colors.black54))),
          TextButton(onPressed: () => Navigator.pop(ctx, true), style: TextButton.styleFrom(backgroundColor: AppColors.error.withValues(alpha: 0.1)), child: Text('Retract', style: GoogleFonts.inter(fontWeight: FontWeight.w800, color: AppColors.error))),
        ],
      ),
    );
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  Text('BROADCAST PROTOCOL', style: GoogleFonts.inter(fontSize: 10, fontWeight: FontWeight.w900, color: isDark ? Colors.white54 : Colors.black54, letterSpacing: 1.0)),
                  const SizedBox(height: 8),
                  Container(
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(hintText: 'Enter broadcast subject', hintStyle: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.w600, color: isDark ? Colors.white38 : Colors.black38), prefixIcon: Icon(Icons.subject_rounded, color: isDark ? Colors.white54 : Colors.black54), border: InputBorder.none, contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 16)),
                    ),
                  ),
                  const SizedBox(height: 16),
                  Container(
                    ...
                    child: TextField(
                      controller: bodyCtrl,
                      maxLines: 4,
                      style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.w500, color: isDark ? Colors.white : Colors.black87),
                      decoration: InputDecoration(hintText: 'Detailed transmission content...', hintStyle: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.w500, color: isDark ? Colors.white38 : Colors.black38), border: InputBorder.none, contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 16)),
                    ),
                  ),
                    ...
                    children: [
                      ...
                      CPPressable(
                        ...
                        child: AnimatedContainer(
                          ...
                          padding: const EdgeInsets.all(12),
                          decoration: BoxDecoration(
                            color: pin ? AppColors.warning.withValues(alpha: 0.15) : (isDark ? Colors.white.withValues(alpha: 0.05) : Colors.black.withValues(alpha: 0.05)),
                            borderRadius: BorderRadius.circular(16),
                            border: Border.all(color: pin ? AppColors.warning : Colors.transparent),
                          ),
                          child: Icon(pin ? Icons.push_pin_rounded : Icons.push_pin_outlined, color: pin ? AppColors.warning : (isDark ? Colors.white54 : Colors.black54), size: 24),
                        ),
                      )
                    ],
                  ),
