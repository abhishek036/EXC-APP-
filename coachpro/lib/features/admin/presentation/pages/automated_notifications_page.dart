...
class AutomatedNotificationsPage extends StatefulWidget {
  ...
  @override
  State<AutomatedNotificationsPage> createState() =>
      _AutomatedNotificationsPageState();
}

class _AutomatedNotificationsPageState
    extends State<AutomatedNotificationsPage> {
  ...
  Future<void> _toggleRule(String id, bool enabled) async {
    final updated = await AutoNotificationService.instance.toggleRule(
      id,
      enabled,
    );
  ...
  Future<void> _updateChannel(String id, String channel) async {
    final updated = await AutoNotificationService.instance.updateChannel(
      id,
      channel,
    );
    setState(() => _rules = updated);
  ...
  Future<void> _triggerManualCheck(String type) async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            '✅ ${type == 'fee' ? 'Fee' : 'Attendance'} notifications triggered',
          ),
          ...
          : ListView(
              ...
              children: [
                ...
                ..._rules
                    .where(
                      (r) =>
                          r.type == AutoNotificationType.feeReminder ||
                ...
                ..._rules
                    .where(
                      (r) => r.type == AutoNotificationType.attendanceAbsent,
                    )
                    .toList()
                    .asMap()
                    .entries
                ...
                ..._rules
                    .where(
                      (r) =>
                          r.type == AutoNotificationType.examReminder ||
            ...
            if (rule.cronExpression != null) ...[
              ...
              Row(
                children: [
                  ...
                  Text(
                    _cronToHuman(rule.cronExpression!),
                    style: GoogleFonts.plusJakartaSans(
                      fontSize: 11,
                      color: CT.textM(context),
                    ),
