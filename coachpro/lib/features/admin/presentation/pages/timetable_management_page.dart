              ...
              DropdownButtonFormField<String>(
                ...
                items: _teachers.map((teacher) {
                  final user = teacher['user'] is Map<String, dynamic>
                      ...
                      : <String, dynamic>{};
                  final name = (teacher['name'] ?? user['name'] ?? 'Teacher')
                      .toString();
                  ...
                  Expanded(
                    child: CPPressable(
                      ...
                      child: Container(
                        ...
                        child: Text(
                          selectedStartTime?.format(ctx) ?? 'Start Time',
                          style: GoogleFonts.plusJakartaSans(
                            color: CT.textH(context),
                          ),
                        ),
                  ...
                  Expanded(
                    child: CPPressable(
                      ...
                      child: Container(
                        ...
                        child: Text(
                          selectedEndTime?.format(ctx) ?? 'End Time',
                          style: GoogleFonts.plusJakartaSans(
                            color: CT.textH(context),
                          ),
                        ),
                ...
                if (batch.isEmpty) {
                  if (mounted) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('Selected batch is no longer available'),
                      ),
                    );
                  }
                  return;
                }
                final days = _normalizeDaysOfWeek(
                  batch['days_of_week'],
                ).toSet();
  ...
  Future<void> _deleteSlot(String batchId) async {
    ...
    if (batch.isEmpty) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Slot could not be removed because the batch was not found',
            ),
          ),
        );
      }
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'This will remove the batch from the selected day.',
        ),
        actions: [
          TextButton(
      ...
      body: Column(
        children: [
                ...
                return CPPressable(
                  ...
                  child: Container(
                    ...
                    child: Text(
                      ...
                      style: GoogleFonts.plusJakartaSans(
                        fontWeight: FontWeight.w600,
                        color: isActive ? Colors.white : CT.textH(context),
                      ),
                    ),
                  ),
                );
          ...
          Expanded(
            child: _isLoading
                ? Center(
                    child: CircularProgressIndicator(color: CT.accent(context)),
                  )
                : currentSlots.isEmpty
                ? Center(
                    child: Text(
                      'No classes scheduled for $_activeDay',
                      style: GoogleFonts.plusJakartaSans(
