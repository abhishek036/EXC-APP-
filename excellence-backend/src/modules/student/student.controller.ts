...
export class StudentController {
  ...
  getDashboard = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Auto-sync fees to ensure correct generation logic
      try {
        await new FeeRepository().autoSyncStudentFees(instituteId, studentId);
      } catch (_e) {
        // Ignore errors
      }
  ...
  getMyFees = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      try {
        await new FeeRepository().autoSyncStudentFees(req.instituteId!, student.id);
      } catch (_e) {
        // Ignore
      }
      ...
      const records = await prisma.feeRecord.findMany({
        ...
        include: {
          ...
          payments: { select: { amount_paid: true, payment_mode: true, paid_at: true, receipt_number: true, status: true, submitted_at: true } }
        },
