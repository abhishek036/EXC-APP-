...
class _StudentProfilePageState extends State<StudentProfilePage> with SingleTickerProviderStateMixin, ThemeAware<StudentProfilePage> {
  ...
  void _populateControllers(Map<String, dynamic> s) {
    ...
    final parents = <Map<String, dynamic>>[
      if (parentsRaw is List)
        ...parentsRaw.whereType<Map>().map((e) => Map<String, dynamic>.from(e)),
    ];

    final parentStudents = <Map<String, dynamic>>[
      if (parentStudentsRaw is List)
        ...parentStudentsRaw.whereType<Map>().map(
    ...
    final nestedParent = parentFromStudent['parent'] is Map
        ...
        : <String, dynamic>{};
    final directParent = parents.isNotEmpty
        ? parents.first
        : <String, dynamic>{};
  ...
  Future<void> _loadAll({bool silent = false}) async {
    ...
    try {
      ...
      final examsResult = await _adminRepo.getExamResults().catchError(
      ...
      if (mounted) {
        ...
        final attPct = _toInt(
          attData['attendancePercent'] ?? attData['percentage'],
        );
        ...
        final rawBatches =
            (studentMap['student_batches'] as List<dynamic>? ?? [])
                .whereType<Map>()
                .map((e) {
                  final batch = e['batch'] as Map?;
                  final id =
                      (batch?['id'] ?? e['batch_id'] ?? e['batchId'] ?? '')
                          .toString();
        ...
        try {
          allBatches = await _adminRepo.getBatches();
        } catch (_) {}
        setState(() {
          _student = studentMap;
          ...
          _examResults = examsResult
              .where(
                (e) =>
                    (e['studentId'] ?? e['student_id'] ?? '').toString() ==
                    widget.studentId,
              )
              .toList();
      ...
      debugPrint(
        'StudentProfilePage _loadAll failed for ${widget.studentId}: $error',
      );
  ...
  Future<void> _removeBatch(Map<String, dynamic> batch) async {
    ...
    try {
      await _adminRepo.removeStudentFromBatch(
        batchId: batchId,
      ...
      if (mounted) {
        setState(() {
          final exists = _studentBatchData.any(
  ...
  Future<void> _showAddBatchSheet() async {
    final assignedIds = _studentBatchData
        .map((b) => b['id'].toString())
        .toSet();
    final available = _allBatches
        .where((b) => !assignedIds.contains(b['id'].toString()))
        .toList();
    ...
    try {
      await _adminRepo.assignStudentToBatch(
        batchId: batchId,
        studentId: widget.studentId,
      );
  ...
  Widget build(BuildContext context) {
                          ...
                          child: CPPressable(
                            onTap: () async {
                              await Clipboard.setData(
                                ClipboardData(text: _loadErrorDetails),
                              );
                              if (mounted) {
                                _showSnack(
                                  'Error copied',
    ...
    final feeStatus = (s['feeStatus'] ?? (s['fee_status'] ?? 'PENDING'))
        .toString()
        .toUpperCase();
                ...
                ..._studentBatchData.asMap().entries.map((entry) {
                  ...
                  return Column(
                    children: [
                      ...
                      if (entry.key < _studentBatchData.length - 1)
                        _dividerLine(),
                    ],
                  );
                }),
  ...
  Widget _buildFeesTab(String feeStatus, Color feeColor) {
    final totalPaid = _feeHistory
        .where((f) => (f['status'] as String?)?.toUpperCase() == 'PAID')
        .length;
    final totalDue = _feeHistory
        .where((f) => (f['status'] as String?)?.toUpperCase() != 'PAID')
        .length;
    final totalAmount = _feeHistory.fold<double>(
      0,
      (sum, f) => sum + _toDouble(f['amount']),
    );
        ...
        children: [
          ...
          if (_feeHistory.isEmpty)
            ...
            ..._feeHistory.asMap().entries.map((entry) {
              ...
              final month =
                  (fee['month'] ?? fee['period'] ?? 'Month ${entry.key + 1}')
                      .toString();
